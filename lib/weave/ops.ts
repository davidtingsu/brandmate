...
export const searchMemoriesOp = weave.op(async function searchMemoriesOp({
  query,
  niche,
  postType,
}: {
...
export const storeLessonOp = weave.op(async function storeLessonOp({
  task,
  niche,
  lesson,
  score_before,
...
export const logHumanFeedback = weave.op(async function logHumanFeedback({
  ...
  scoreBefore?: number;
}): Promise<{ traceId?: string; feedbackType: HumanFeedbackType; scoreBefore?: number }> {
  await ensureWeave();
...
export const orchestratePostLoop = weave.op(async function orchestratePostLoop(
  ...
  const { lessons } = await searchMemoriesOp({
    query: input.topic,
    niche,
    postType,
  });
